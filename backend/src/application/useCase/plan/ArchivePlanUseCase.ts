...
export class ArchivePlanUseCase implements IArchivePlanUseCase {
  ...
  async execute(planId: string): Promise<PlanResponseDTO> {
    ...
    if (existingPlan.stripe?.productId) {
      await this._billingCatalog.archiveBillingCatalog({
        productId: existingPlan.stripe.productId,
        ...(existingPlan.stripe.monthlyPriceId
          ? { monthlyPriceId: existingPlan.stripe.monthlyPriceId }
          : {}),
        ...(existingPlan.stripe.yearlyPriceId
          ? { yearlyPriceId: existingPlan.stripe.yearlyPriceId }
          : {}),
      });
    }

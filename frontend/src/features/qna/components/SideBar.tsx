...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex w-64 shrink-0 border-r border-white/5 bg-[#0B0B0D]/60 backdrop-blur-xl min-h-full sticky top-0 flex-col z-10">
      <nav className="flex-1 space-y-1 px-3 py-6">
          ...
          >
            {({ isActive }) => (
              <>
                ...
                <span className={`transition-colors duration-300 flex items-center justify-center ${isActive ? 'text-indigo-400' : 'text-zinc-400 group-hover:text-zinc-300'}`}>
                  {item.icon}
                </span>
                {item.title}
              </>
        ...
        <div className="mt-8 pt-6 border-t border-white/5 w-full space-y-1">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest px-4 mb-3">
            Personal
          </p>

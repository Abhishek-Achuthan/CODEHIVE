...
export const RoomList: React.FC<RoomListProps> = ({ rooms, isLoading, error }) => {
    if (isLoading) {
        return (
    ...
    if (!rooms || rooms.items.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 text-center animate-in fade-in zoom-in duration-500">
                ...
                <h3 className="text-2xl font-bold text-white mb-3">No public rooms available</h3>
                <p className="text-zinc-500 max-w-sm">
                    Be the first to create one! Head back to the landing page to start your own room.
                </p>
            </div>
        );
    }

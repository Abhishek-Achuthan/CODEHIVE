        ...
        <div className="min-h-screen bg-black text-white">
            ...
            <main className="px-4 py-8">
                <div className="mx-auto max-w-5xl space-y-8">
                    <button onClick={() => navigate(-1)} className="flex items-center text-zinc-400 hover:text-white transition-colors group">
                    ...
                    <div className="bg-zinc-900/80 border border-zinc-800 rounded-2xl p-6 backdrop-blur-sm flex items-start gap-6 shadow-lg shadow-black/20">
                        ...
                        <div>
                            ...
                            <p className="text-zinc-400 text-lg">{mentor?.title || "Expert Mentor"}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-[380px_1fr] gap-8">
                        ...
                        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 flex flex-col">
                            <h3 className="text-lg font-semibold mb-6 flex items-center gap-2">
                                ...
                                Available Slots
                                {selectedDate && <span className="text-sm font-normal text-zinc-500 ml-2">for {selectedDate.toLocaleDateString()}</span>}
                            </h3>
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                            {slots.map((slot, idx) => (
                                                <button
                                                    ...
                                                    `}
                                                >
                                                    <div className="text-base font-semibold mb-1">{slot.startTime}</div>
                                                    <div className="text-xs opacity-75">to {slot.endTime}</div>
                                                    <div className="absolute top-4 right-4 text-sm font-bold opacity-60">₹{slot.price}</div>
                                                </button>
                                            ))}
                                        </div>
                                    </div>
...
What would you like to discuss?
                                    ...
                                    <p className="text-zinc-400 text-sm">
                                        ...
                                        This helps them prepare for the session.
                                    </p>
                                    <div className="flex items-center gap-4 p-4 bg-zinc-950/50 rounded-xl border border-zinc-800/50">
                                        <div>
                                            <div className="text-xs text-zinc-500 uppercase font-bold">Session</div>
                                            <div className="text-white font-medium">{selectedDate?.toLocaleDateString()} • {selectedSlot.startTime}</div>
                                        </div>
                                        <div className="h-8 w-px bg-zinc-800"></div>
                                        <div>
                                            <div className="text-xs text-zinc-500 uppercase font-bold">Price</div>
                                            <div className="text-white font-medium">₹{selectedSlot.price}</div>
                                        </div>
                                    </div>
                                </div>

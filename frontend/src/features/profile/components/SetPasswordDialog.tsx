...
import { useSetPassword } from "../../auth/hooks/useSetPassword";
...
export default function SetPasswordDialog({
  ...
  const { setPassword, saving } = useSetPassword(() => onOpenChange(false));
  ...
  const submit = async () => {
    await setPassword({ newPass, confirmNewPass });
  };
